...
saveRDS(df_comparisons_plot,'./df_comparisons_plot_merged.rds')
...
ggsave(
  filename = paste0("./treg_better_JSD_merged_nolabel.png"),
  plot = p,
  width = 8,
  height = 6,
  dpi = 300,
...
pooled_better = unique(dfp %>% dplyr::filter(tregs_better_cohens==1) %>% dplyr::pull(param_set_id))
pooled_worse  = unique(dfp %>% dplyr::filter(tregs_better_cohens==-1) %>% dplyr::pull(param_set_id))
