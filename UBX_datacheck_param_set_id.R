...
# rep_ind_vec  = setdiff(0:99,c(34,45,54,61,80))
...
ggsave(
  filename = paste0("./timeseries_",param_id,".png"),
  plot = p,
  width = 8,
  height = 5,
  dpi = 300,
...
mean(results_000$epithelial_score)-mean(results_010$epithelial_score)

mean(results_000$epithelial_score)-mean(results_010$epithelial_score)
calculate_js_divergence(results_010$epithelial_score, 
                        results_000$epithelial_score, 
                        method = "fd")[1]

# df_results_keep    = readRDS('/Users/burcutepekule/Dropbox/tregs_ubelix/data_cpp_read_cohens.rds')
# df_results_pick_s = df_results_keep %>% dplyr::filter(injury_type=='sterile' & param_set_id==param_id & replicate_id %in% rep_ind_vec)
# df_results_pick_p = df_results_keep %>% dplyr::filter(injury_type=='pathogenic' & param_set_id==param_id & replicate_id %in% rep_ind_vec)


## =====================================

df_params = read_csv('/Users/burcutepekule/Dropbox/tregs_ubelix/lhs_parameters_ubelix.csv', show_col_types = FALSE)
# df_params_pick = df_params %>% dplyr::filter(param_set_id %in% c(5, 100, 19605, 45003, 1503))
# df_params_pick = df_params %>% dplyr::filter(param_set_id %in% c(15202, 19002, 27004, 29510, 67500))
df_params_pick = df_params %>% dplyr::filter(param_set_id %in% param_id)

df_params_pick$epith_recovery_chance
