...
p_values_for_plot = param_tests %>%
  ...
  dplyr::select(parameter, adj_p_better_vs_worse, adj_p_better_vs_neutral, adj_p_worse_vs_neutral) %>%
  mutate(
    p_label_1 = case_when(
      adj_p_better_vs_worse < 0.001 ~ "***",
      adj_p_better_vs_worse < 0.01 ~ "**",
      adj_p_better_vs_worse < 0.05 ~ "*",
      TRUE ~ "ns"
    ),    
    p_label_2 = case_when(
    ...
    p_label_3 = case_when(
      adj_p_worse_vs_neutral < 0.001 ~ "***",
      adj_p_worse_vs_neutral < 0.01 ~ "**",
      adj_p_worse_vs_neutral < 0.05 ~ "*",
      TRUE ~ "ns"
    ),
    p_text_1 = sprintf("p_bvw = %.2e %s", adj_p_better_vs_worse, p_label_1),
    p_text_2 = sprintf("p_bvn = %.2e %s", adj_p_better_vs_neutral, p_label_2),
    p_text_3 = sprintf("p_wvn = %.2e %s", adj_p_worse_vs_neutral, p_label_3),
    # Combine into one string with line breaks
    p_text = paste(p_text_1, p_text_2, p_text_3, sep = "\n")
  )
...
ggsave(
  filename = paste0("./treg_top_params_PLS_DA_",inj_type,"_da_regions.png"),
  plot = p_params,
  width = 18,
  height = 20,
  dpi = 300,
