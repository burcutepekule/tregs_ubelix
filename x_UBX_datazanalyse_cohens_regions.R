...
tol_in              = 125*0.05 # this is NOT over epithelial cells (so don't use 25*stg), it's over epithelial score! max diff is 6*25-1*25=125!
cohens_d_th         = 0.5
sd_tol_in           = 15
...
df_comparisons_sense = df_comparisons %>% dplyr::mutate(makes_sense=ifelse(sign(diff_tregs_on_minus_off)!=sign(diff_notrnd_minus_rnd) & abs(diff_notrnd_minus_rnd)>tol_in
                                                                           & (abs(d_21)>=0.5 | abs(d_54)>=0.5) ,0,1)) #either for sterile or pathogenic
...
df_comparisons_plot_pathogenic = df_comparisons_plot_pathogenic %>% dplyr::mutate(tregs_better_cohens = ifelse(abs(d_10)>cohens_d_th, tregs_better, 0))
df_comparisons_plot_pathogenic = df_comparisons_plot_pathogenic %>% dplyr::mutate(tregs_better_cohens_sd = ifelse(tregs_on_no_rnd_sd<sd_tol_in
                                                                                                                  & tregs_off_sd<sd_tol_in,
                                                                                                                  tregs_better, 0))

df_comparisons_plot_pathogenic = df_comparisons_plot_pathogenic %>% select(-d_43)
...
df_comparisons_plot_sterile = df_comparisons_plot_sterile %>% dplyr::mutate(tregs_better = ifelse(diff_tregs_on_minus_off > tol_in, 1,
                                                                                                  ifelse(diff_tregs_on_minus_off < -1*tol_in,-1,0)))
df_comparisons_plot_sterile = df_comparisons_plot_sterile %>% dplyr::mutate(tregs_better_cohens = ifelse(abs(d_43)>cohens_d_th, tregs_better, 0))
df_comparisons_plot_sterile = df_comparisons_plot_sterile %>% dplyr::mutate(tregs_better_cohens_sd = ifelse(tregs_on_no_rnd_sd<sd_tol_in
                                                                                                            & tregs_off_sd<sd_tol_in,
                                                                                                            tregs_better, 0))

df_comparisons_plot_sterile = df_comparisons_plot_sterile %>% select(-d_10)
...
cohens_th = cohens_d_th # example threshold for x
...
dfp = dfp %>%
  mutate(color_group = case_when(
    x >= cohens_th & y >= e_th ~ as.character(param_set_id),  # blue region
    x >= cohens_th & y <= -e_th ~ as.character(param_set_id), # pink region
...
dfp = dfp %>%
  mutate(region = case_when(
    x >= cohens_th & y >= e_th ~ "blue_region",  # blue region
    x >= cohens_th & y <= -e_th ~ "pink_region", # pink region
    TRUE ~ "other"
  ))
...
blue_colors = setNames(colorRampPalette(c("#016B61","#4FB7B3","#637AB9", "#3B38A0", "#1A2A80"))(length(blue_params)), 
                       as.character(blue_params))
pink_colors = setNames(colorRampPalette(c("#FA812F","#E83F25","#E06B80","#F75270", "#DC143C"))(length(pink_params)), 
                       as.character(pink_params))

# Combine all colors
all_colors = c("other" = "gray50", blue_colors, pink_colors)
...
p = ggplot(dfp, aes(x = x, y = y, shape = injury_type, color = color_group)) +
  ...
  theme_minimal() +
  scale_x_continuous(breaks = c(0, cohens_th, seq(1, ceiling(max(dfp$x)), by = 10))) +
  labs(x = paste0("|cohen's d| (threshold = ", cohens_th, ")"),
       y = paste0("Epithelial score for tregs_on - tregs_off (threshold = ±", e_th, " for a total of 25 cells.)"),
       shape = "Injury Type",
       color = "Parameter Set ID") + 
  # scale_x_log10()
  scale_x_continuous(
    trans = scales::pseudo_log_trans(base = 10, sigma = 1),
    breaks = c(0, cohens_th, 1, 5, 10, 20, 50, 100)
  )+guides( # 3 columns for legends
    color = guide_legend(ncol = 3),
    shape = guide_legend(ncol = 3)
  )

ggsave(
  filename = paste0("./treg_better_cohens.png"),
  plot = p,
  width = 9,
  height = 6,
  dpi = 300,
...
dfp_p = dfp %>% dplyr::filter(injury_type=='pathogenic') %>% select(param_set_id, tregs_better_cohens)
dfp_s = dfp %>% dplyr::filter(injury_type=='sterile') %>% select(param_set_id, tregs_better_cohens)
...
df_comparisons_plot_nz = df_comparisons_plot %>% dplyr::filter(tregs_better_cohens_sd!=0)
