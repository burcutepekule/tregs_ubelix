...
plsda_df = data.frame(
  Comp1 = plsda_proj[, 1],
  Comp2 = plsda_proj[, 2],
  treg_outcome = as.factor(abs(df_lda$tregs_better_cohens)) # ABS PUTS BOTH IN THE SAME CATEOGRY!
)
...
# Get parameter sets that fall in "Tregs matter" region
...
tregs_matter_cov  = cov(tregs_matter_params)
...
ggsave(
  filename = paste0("./treg_regions_PLS_DA_",inj_type,"_w_arrows_tregs_matter.png"),
  plot = p2,
  width = 9,
  height = 6,
  dpi = 300,
...
ggsave(
  filename = paste0("./treg_regions_PLS_DA_",inj_type,"_w_arrows_tregs_matter_new_points.png"),
  plot = p2,
  width = 9,
  height = 6,
  dpi = 300,
...
# Export
output_file = "./tregs_matter_parameters_ubelix.csv"
