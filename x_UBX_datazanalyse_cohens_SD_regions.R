...
df_comparisons_below= df_comparisons %>% dplyr::filter((injury_type=='pathogenic' & abs(d_10)<cohens_d_th)|(injury_type=='sterile' & abs(d_43)<cohens_d_th))
...
df_comparisons_plot_pathogenic = df_comparisons_plot_pathogenic %>% dplyr::mutate(tregs_better_cohens = ifelse(abs(d_10)>cohens_d_th
...
df_comparisons_plot_sterile = df_comparisons_plot_sterile %>% dplyr::mutate(tregs_better = ifelse(diff_tregs_on_minus_off > tol_in, 1,
                                                                                                  ifelse(diff_tregs_on_minus_off < -1*tol_in,-1,0)))
df_comparisons_plot_sterile = df_comparisons_plot_sterile %>% dplyr::mutate(tregs_better_cohens = ifelse(abs(d_43)>cohens_d_th
                                                                                                         & tregs_on_no_rnd_sd<sd_tol_in
                                                                                                         & tregs_off_sd<sd_tol_in,
                                                                                                         tregs_better, 0))
...
# ============CONFLICT?=========================================================
tregs_better_sterile = df_comparisons_plot_sterile %>% dplyr::filter(tregs_better==1) %>% dplyr::pull(param_set_id)
tregs_worse_sterile  = df_comparisons_plot_sterile %>% dplyr::filter(tregs_better==-1) %>% dplyr::pull(param_set_id)
tregs_better_pathogenic = df_comparisons_plot_pathogenic %>% dplyr::filter(tregs_better==1) %>% dplyr::pull(param_set_id)
tregs_worse_pathogenic  = df_comparisons_plot_pathogenic %>% dplyr::filter(tregs_better==-1) %>% dplyr::pull(param_set_id)
...
p = ggplot(dfp, aes(x = x, y = y, shape = injury_type, color = color_group)) +
  ...
  scale_x_continuous(
    ...
    breaks = c(0, cohens_th, 1, 2, 3, 5, 10, 20, 50, 100)
...
ggsave(
  filename = paste0("./treg_better_cohens_SD.png"),
  plot = p,
  width = 9,
  height = 6,
  dpi = 300,
